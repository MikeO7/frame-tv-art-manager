...
func Load() (*Config, error) {
	cfg := &Config{
		...
		ConnectionTimeout:   time.Duration(envInt("CONNECTION_TIMEOUT_SECONDS", 30)) * time.Second,
		APITimeout:          time.Duration(envInt("API_TIMEOUT_SECONDS", 30)) * time.Second,
		UploadDelay:         time.Duration(envInt("UPLOAD_DELAY_MS", 3000)) * time.Millisecond,
		UploadAttempts:      envInt("UPLOAD_ATTEMPTS", 3),
		GateTimeout:         time.Duration(envInt("GATE_TIMEOUT_MS", 5000)) * time.Millisecond,
		PUID:                envInt("PUID", 0),
		PGID:                envInt("PGID", 0),
	}

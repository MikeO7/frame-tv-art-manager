...
func (env *environment) loadConfig() (*Config, error) {
	cfg := &Config{
		...
		SmartCropProtection:            env.boolean("SMART_CROP_PROTECTION", true),
		SmartCropProtectionStrength:    env.decimal("SMART_CROP_PROTECTION_STRENGTH", 0.35),
		SmartCropProvider:              strings.ToLower(env.str("SMART_CROP_PROVIDER", smartCropProviderLocal)),
		SmartCropProviderURL:           env.str("SMART_CROP_PROVIDER_URL", ""),
		SmartCropProviderMinConfidence: env.decimal("SMART_CROP_PROVIDER_MIN_CONFIDENCE", 0.7),
		SmartCropProviderTimeout:       safeDuration(env.integer("SMART_CROP_PROVIDER_TIMEOUT_SECONDS", 8), time.Second),
		...
		ColorProfilePolicy:             strings.ToLower(env.str("IMAGE_COLOR_PROFILE_POLICY", colorProfileConvertSRGB)),

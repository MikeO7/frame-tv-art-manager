...
## 2026-07-06 - Context alignment for Samsung TV Client Network Methods
**Learning:** Core connection and request methods in the Samsung TV client (`turnOffTV`, `checkArtModeGate`, `fetchDeviceInfo`, `ensureToken`, `getCategories`, `sendArtRequest`) lack thorough GoDoc headers, particularly missing explicit `Parameters` and `Returns` breakdowns which can lead to context decay during future network refactors.
**Action:** When acting as Scribe to document Go APIs, always ensure precise `Parameters:` and `Returns:` sections are explicitly mapped above un-documented utility methods to self-document the type boundaries. Always run `make test` rather than `make check` if un-related linter rules (like `lll` and `depguard`) block validation on unmodified files.

...
## 2026-06-21 - Context alignment for undocumented connection methods
**Learning:** Scribe constraints require strictly checking files (like `.env.example`) to see if keys are actually missing before proposing changes. In this trace, I learned to rely on the active file state (`grep -C 3 "LOCATION" .env.example`) instead of assuming from memory. Additionally, I learned to extract multiple undocumented internal wrapper methods (like `checkGate`, `setupToken`, `ensureToken`, `fetchDeviceInfo`, `turnOffTV`, `checkArtModeGate`) and meticulously verify their implementation to correctly attach parameter bounds and GoDoc summaries using automated replacement scripts, verifying they pass CI via `make check`.
**Action:** Always verify current file contents and `cat` target files before submitting a plan, explicitly satisfying the Groundedness Rule instead of relying on memory, and always run `make check` to verify documentation syntax doesn't introduce CI errors.

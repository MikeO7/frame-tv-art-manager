...
## 2026-06-02 - Fast-Path Integer Accumulation for Contrast Scaling
**Learning:** In tight per-pixel loop aggregations, casting pixel values to `float64` inside the loop causes massive CPU overhead due to IEEE floating point operations.
**Action:** When calculating luminance or color channels across large images, use integer math (e.g., `uint32` or `uint64`) scaled up to hold fractional weights (e.g., `* 299` instead of `* 0.299`). Accumulate inside the loop as integers, and defer the floating point conversion and scaling (division by the weight multipliers) until strictly after the loop completes. This eliminates floating point operations on a per-pixel basis and scales highly for large image optimizations.

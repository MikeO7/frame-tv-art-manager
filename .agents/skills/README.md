...
Vendor-neutral engineering workflows stored in the Agent Skills directory
convention. Assistants with native skill discovery can load each `SKILL.md`;
other assistants can read the same files directly.

Skills may describe optional facilities such as background or parallel agents.
When a platform does not provide them, execute the workflow sequentially while
preserving its review and verification requirements.
